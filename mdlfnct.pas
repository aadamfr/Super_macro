...
Procedure ScruteFichier(Dossier:string;filtre:string;Attributs:integer; ToFile : String); overload;
Procedure ScruteFichier(Dossier:string;filtre:string;Attributs:integer; ToList : TStrings); overload;
...
function GetDriverInfo(Driver : string; info : cardinal) : string;
...
procedure KeyB(Key : cardinal; Down : Boolean = True);
procedure ExtraASCII(Code :cardinal);
procedure CloseFileToListFile(var Tab : TListFile; Name : string);
...
var i : integer;
begin
for i := Low(Tab.Name) to High(Tab.Name)
do if ListFile.Name[i] = Name
   then begin
        if (Tab.Index[i]) > 0
        then Tab.Index[i] := (Tab.Index[i] * -1)
        else Exit;
        CloseFile(Tab.MemFile[i]);
        break;
        end;
end;
...
var
  ...
  dType       : cardinal;
  SectPerCls,BytesPerCls,FreeCls,TotCls,BytesPerCluster  : DWord;
...
if (length(Driver) > 0)
then begin
     if not (Driver[1] in ['a'..'z','A'..'Z'])
     then begin Form1.ErrorComportement('Lettre de lecteur invalide.');Exit;end;
     end
else begin Form1.ErrorComportement('Lettre de lecteur invalide.');Exit;end;
  ...
  BytesPerCluster  := SectPerCls * BytesPerCls;
case info of
  0: begin
     dType := GetDriveType(PChar(drv));
    case dType of
         DRIVE_TYPE_UNDTERMINED : result := 'Unable to Determine';
         DRIVE_ROOT_NOT_EXIST   : result := 'Does not exist';
         DRIVE_REMOVABLE    : result := 'Removable Drive (Floppy)';
         DRIVE_FIXED         : result := 'Fixed Disk';
         DRIVE_REMOTE        : result := 'Remote or Network Drive';
         DRIVE_CDROM         : result := 'CD-ROM Drive';
         DRIVE_RAMDISK      : result := 'RAM Drive';
    end;
    end;

  1 : result := FormatFloat('0.00', ( BytesPerCluster *(TotCls /1000000)));
  2 : result := FormatFloat('0.00', ( BytesPerCluster *(FreeCls/1000000)));
  end;
...
procedure KeyB(Key : cardinal; Down : Boolean = True);
begin
if not (Key in[VK_CONTROL,VK_MENU,VK_SHIFT])
then begin
     if Down = True
     then KeyBD_event(key, MapvirtualKey( Key, 0) , KEYEVENTF_EXTENDEDKEY or 0, 0)
     else KeyBD_event(key, MapvirtualKey( Key, 0) , KEYEVENTF_EXTENDEDKEY or KEYEVENTF_KEYUP, 0);
     end
else begin
     if Down = True
     then KeyBD_event(Key, 0, 0, 0)
...
begin
  try
    ...
    Reset(F);
    if not eof(F)
    then Read(F,sign[0],sign[1],sign[2]);
...
Procedure ScruteFichier(Dossier:string;filtre:string;Attributs:integer; ToList : TStrings);
var FichierTrouve:string;
    Resultat:Integer;
    SearchRec:TSearchRec;
    MaskPtr,Ptr:Pchar;
...
Procedure ScruteDossier(Penetration:integer;Dossier:string;filtre:string;attributs:integer;recursif:boolean; ToFile : String); overload;
...
function FileNameValide(FileName : string) : boolean;
begin
// exlusion des caractères impossibles dans les noms de fichier
result := False;
if Pos('/',FileName) <> 0 then exit;
//if not((Pos(':',FileName) = 2) and (Pos(':',FileName) = 0)) then exit;
if Pos('*',FileName) <> 0 then exit;
if Pos('?',FileName) <> 0 then exit;
if Pos('"',FileName) <> 0 then exit;
if Pos('<',FileName) <> 0 then exit;
...
procedure touche_altGr(touche_supp : integer);
begin
KeyB(VK_CONTROL,True);
touche_Alt(touche_supp);
KeyB(VK_CONTROL,False);
end;
...
if CapLock = false
then KeyB(vk_Lshift,True);
KeyB(touche_supp, True);
if CapLock = false
then KeyB(vk_Lshift,False);
end;
...
var  ExtSpl : boolean;
     ...
     TabOfInc : TTabOfInclude;
const IndentationMaxi = 14;
begin
ExtSpl := False;
...
then if Fichier[1] ='\'
     then begin
          if ALIAS_EXE = ''
          then tmp := ExtractFileDir(form1.StatusBar1.Panels[0].Text)+'\'
          else tmp := ExtractFileDir(ALIAS_EXE)+'\';
          Fichier := StringReplace(Fichier,'\',tmp,[]);
          end;
...
then begin
     ...
     if SousMacroIndent >= IndentationMaxi // en réalité 12 mais l'incrémentation se fait après le test
     then begin
          Form1.ErrorComportement('Niveau d''indentation de sous macro trop grand (Max '+IntToStr(IndentationMaxi)+').',1);
          exit;
          end;
     if not FileExists(Fichier)
     ...
     Inc(SousMacroIndent);
     if length(TabSousMacroExecute) < SousMacroIndent
     then begin
          Form1.ErrorComportement('Niveau d''indentation de sous macro trop grand (Max '+IntToStr(IndentationMaxi)+') Erreur inattendue.',1);
          Exit;
          end;
     ...
     Form1.LoadTabInclude(TabOfInc,SousMacro);
     ...
     if (Form1.CheckBox3.Checked = True) or (Form19.CheckBox5.Checked = True)
     then begin
          Form1.RichEdit1.Lines.Add('');
          Form1.RichEdit1.SelText :=black('Entre dans une sous-macro ('+ExtractFileName(Fichier)+')['+IntToStr(SousMacroIndent)+'/'+IntToStr(IndentationMaxi)+'].');
          end;
     ...
     do begin
        ...
        then if SousMacro.Items[i].Caption = 'Quitter' then break;
            ...
            Form1.RestoreTabInclude(TabOfInc,SousMacro); 
            Dec(SousMacroIndent);
            if (SousMacroIndent >= Low(TabSousMacroExecute)) and (SousMacroIndent <= High(TabSousMacroExecute))
            then SousMacroExecute := TabSousMacroExecute[SousMacroIndent]
            else SousMacroExecute := nil;

            if (SousMacroIndent >= Low(TabSousMacroExecute)+1) and (SousMacroIndent <= High(TabSousMacroExecute)+1)
            then TabSousMacroExecute[SousMacroIndent+1] := nil;
            SousMacro.Free;
            if (Form1.CheckBox3.Checked = True) or (Form19.CheckBox5.Checked = True)
            then begin
                 Form1.RichEdit1.Lines.Add('');
                 Form1.RichEdit1.SelText :=black('Quitte la sous-macro ('+ExtractFileName(Fichier)+')['+IntToStr(SousMacroIndent)+'/'+IntToStr(IndentationMaxi)+'].');
                 end;
            if FileExists(UpdateIniFile)
...
do begin
        KeyBD_event(VK_NUMPAD0+StrToInt(Str[i]), MapvirtualKey(VK_NUMPAD0+StrToInt(Str[i]), 0) , 0, 0);
        KeyBD_event(VK_NUMPAD0+StrToInt(Str[i]), MapvirtualKey(VK_NUMPAD0+StrToInt(Str[i]), 0), KEYEVENTF_KEYUP, 0);
end;
KeyB(VK_MENU,False);
Application.ProcessMessages;
end;
...
then begin
     ...
     if Sp_Key[1] <> 0 then KeyB(Sp_key[1],True);
     if Sp_Key[2] <> 0 then KeyB(Sp_key[2],True);
     if Sp_Key[3] <> 0 then KeyB(Sp_key[3],True);
     Exit;
     end;
// Action relâché
if listParam.param[ListParam.nbr_param-1] = '[KeyUp]'
then begin
     if Sp_Key[3] <> 0 then KeyB(Sp_key[3],False);
     if Sp_Key[2] <> 0 then KeyB(Sp_key[2],False);
     if Sp_Key[1] <> 0 then KeyB(Sp_key[1],False);
     Application.ProcessMessages;
     Exit;
     end;
...
else form1.ErrorComportement('La valeur contenue dans '+ListParam.Param[1]+'n''est pas un numérique valide ['+GetValue(ListParam.Param[1])+']');
if Type_var2 = TNum
then if FnctIsInteger(GetValue(ListParam.Param[3])) = True
then i_var2 := StrToInt(GetValue(ListParam.Param[3]))
else form1.ErrorComportement('La valeur contenue dans '+ListParam.Param[3]+'n''est pas un numérique valide ['+GetValue(ListParam.Param[3])+']');
...
then begin
     ...
     if (ListParam.Param[3] = 'Caractère(s)/Longueur')
     then begin
          if FnctIsInteger(GetValue(ListParam.param[5]))
          then nbr1 := StrToInt(GetValue(ListParam.param[5]))
          else begin Form1.ErrorComportement('Le paramètre 5 de la commande Caractère(s)/Longueur est invalide ['+ListParam.param[5]+'].');Exit;end;
          if FnctIsInteger(GetValue(ListParam.param[6]))
          then nbr2 := StrToInt(GetValue(ListParam.param[6]))
          else begin Form1.ErrorComportement('Le paramètre 6 de la commande Caractère(s)/Longueur est invalide ['+ListParam.param[6]+'].');Exit;end;
          Val := Copy(GetValue(ListParam.param[4]),nbr1,nbr2);
          goto Affectation;
          end;
     ...
     if (ListParam.Param[3] = 'Explode')
     then begin
          Explode(GetValue(Param4),GetValue(ListParam.param[5]),StrArray);
          if not FnctIsInteger(GetValue(ListParam.param[6])) then begin Form1.ErrorComportement(GetValue(ListParam.param[6])+' n''est pas une valeur numerique valide.',1); exit; end;
          if length(StrArray) >= StrToInt(GetValue(ListParam.param[6]))
          then Val := StrArray[StrToInt(GetValue(ListParam.param[6]))-1]
          else Val := '';
          goto Affectation;
          end;
     end;
...
then begin
     ...
     then begin val := ClipBoard.AsText; goto Affectation; end;
     if (ListParam.Param[3] = 'Format')
     then begin val := IntToStr(Form1.GetClipboardFormat); goto Affectation; end;
     end;
...
then begin
     ...
     then begin
          ...
          then nbr1 := StrToInt(param4)
               ...
               Form1.ErrorComportement('Le paramètre de décalage de jour n''est pas une valeur numérique. ['+param4+']');
               Exit;
               end;
          end;
...
if (ListParam.Param[2] = 'Disque')
then begin
     if ListParam.Param[3] ='Type de disque' then Val := GetDriverInfo(chaine,0);
     if ListParam.Param[3] ='Taille Totale' then Val := GetDriverInfo(chaine,1);
     if ListParam.Param[3] ='Taille Disponible' then Val := GetDriverInfo(chaine,2);
     goto Affectation;
     end;
...
then begin
     ...
     then begin P := PChar(ExtractFilePath(application.ExeName)); Val := String(P); goto Affectation; end;
...
then if FileName[1] ='\'
     then begin
          ...
          FileName := StringReplace(FileName,'\',tmp,[]);
          end;
...
then begin
     ...
     // réouverture a la bonne position
     if ListFile.Index[index] < 0
     then begin
          assignfile(ListFile.memFile[index],FileName);
          FileMode := 0;
          reset(ListFile.memFile[index]);
          ListFile.Index[index] := abs(ListFile.Index[index]);
          for j := 0 to ListFile.Index[index]-1
          do readln(ListFile.memFile[index],valeur);
          end;
...
then begin
     ...
     CloseFileToListFile(ListFile,FileName);
...
if ListParam.Param[2] = 'Fermeture forcée'
then begin
...
then begin
     GetWindowPlacement(handle,@Wplacement);
     if  Uint(Wplacement.showCmd) = SW_SHOWMAXIMIZED
     then ShowWindow(handle, SW_SHOWNORMAL);
...
then begin
     if ExtractFileExt(EditCible) = ''
     then FileDir := EditCible
     else FileDir := ExtractFileDir(EditCible)+'\';
     if not DirectoryExists(FileDir)
     then ForceDirectories(FileDir);

     if FileExists(EditSource)
     then begin
          if ListParam.param[1] = 'Copier'
          then begin
               if (ExtractFileExt(EditCible) = '') and (EditCible[length(EditCible)] <> '\')
               then begin
                    EditCible := EditCible + '\';
                    EditCible := EditCible + ExtractFileName(EditSource);
                    end;
               RetourVal := CopyFile(PChar(EditSource),PChar(EditCible),False);
               end;
          ...
          if RetourVal = False then Form1.ErrorComportement(SysErrorMessage(GetLastError)+'[Src='+EditSource+', Dest='+EditCible+']');
          end;
     end;
...
then begin
     ...
     then begin
          if Not FileExists(GetValue(ListParam.Param[4]))
          then begin // création du fichier même si pas de résultat
               assignfile(Fichier,GetValue(ListParam.Param[4]));
               rewrite(Fichier);
               CloseFile(Fichier);
               end;
          ScruteDossier(0,GetValue(ListParam.Param[2]),GetValue(ListParam.Param[3]),faAnyFile,True,GetValue(ListParam.Param[4]));
          end
     else Form1.ErrorComportement('Nom de fichier invalide. ('+GetValue(ListParam.Param[4])+')',1);
     end;
end;
...
if ListParam.Param[1] = 'Renommer'
then begin
     NewName := GetValue(ListParam.Param[3]);
     with shellinfo
     do begin
        Wnd    := 0;
        wFunc  := FO_RENAME;
     ...
     if 0 <> SHFileOperation(shellinfo)
     then Form1.ErrorComportement('Renommage du répertoire echoué ['+PathName+','+NewName+'].',3);
    end;
