...
begin
    ...
    begin
        if _ProcessorsCount < 0 then CollectCPUData;
        if (Index >= _ProcessorsCount) or (Index < 0)
            then Form1.ErrorComportement('CPU index out of bounds',4);
        if _PrevSysTime = _SysTime then result:=0 else
        result:=1-(_Counters[index] - _PrevCounters[index])/(_SysTime-_PrevSysTime);
    end else
    begin
        if Index <> 0 then
            Form1.ErrorComportement('CPU index out of bounds',4);
...
begin
    ...
    begin
        ...
        if _POT.ObjectNameTitleIndex <> Processor_IDX then
            Form1.ErrorComportement('Unable to locate the "Processor" performance object',4);
        ...
        if _PCD.CounterNameTitleIndex <> CPUUsageIDX then
            Form1.ErrorComportement('Unable to locate the "% of CPU usage" performance counter',4);
    ...
    begin
        ...
        begin
            ...
            if R <> ERROR_SUCCESS then
                Form1.ErrorComportement('Unable to start performance monitoring',4);
            ...
            if R <> ERROR_SUCCESS then
                Form1.ErrorComportement('Unable to read performance data',4);
...
initialization
    ...
    if not GetVersionEX(VI) then Form1.ErrorComportement('Can''t get the Windows version',4);
